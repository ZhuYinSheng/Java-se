...
public class Main {
  ...
  public static void main(String[] args) {
    while (true) {
      Rules.returnMenu = false;//true为返回主界面，false为不返回主界面

      int playerCount = Rules.playersCountInit();
      Rules.setPlaysCount(playerCount);
      Player[] players = Player.intPlayers(playerCount);

      while (true) {
        Rules.showFingers(players);
        if (Rules.returnMenu) {//如果要回到主界面，则跳出出拳的循环
          break;
        } else {
      ...
      if (Rules.returnMenu) {//如果要回到主界面，则继续外层循环
        continue;
      }
