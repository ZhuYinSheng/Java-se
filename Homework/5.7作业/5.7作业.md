...
abstract class B implements A  //抽象类可以不用实现抽象方法
{ 
    void getData() 
    { 
...
public class Test extends B //必须强制实现两个抽象方法
{ 
    public void myMethod() 
    { 
...
interface A
{
    int i = 111; // public static final int i = 111；常量无法被修改
}
...
```
编译失败，因为A中的 i 是 final 类型的，无法被修改。     
4. 
```
class A
{
...
```
编译失败，接口B不能继承A类，只能继承接口A。也不能继承抽象类，接口只能继承接口。     
5. 
```
interface P
{
...
// one two 两个接口，都有方法的默认实现，则子类必须强制要求重写该方法
// one two three 都有相同方法的实现 
public class MainClass
{
    public static void main(String[] args)
    {
